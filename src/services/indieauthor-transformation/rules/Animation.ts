...
export class AnimationRule extends AbstractRule implements Rule {
    widget = 'Animation';

    private templateWithoutParent =
        "Widget '{{{name}}}': Animation { help '{{{help}}}', { background:  Image { url '{{{image}}}', alt '{{{alt}}}', Original }, pieces: [{{{pieces}}}] }}";
    ...
    do(element: any, parent: any, context: RuleContext): string {
        ...
        return this.renderer.render(template, {
            ...
            image: TransformationUtils.prepareString(element.data.image)
        });
    }
